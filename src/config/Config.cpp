...
namespace Config {
    ...
    none_ initializeApp(CConfReader *conf) {
        ...
        if (conf->readShort(section, "protocol_version",
                            &App::PROTOCOL_VERSION) ||
            0 == App::PROTOCOL_VERSION) {
            App::PROTOCOL_VERSION = DefaultConfig::App::PROTOCOL_VERSION;
        }

        if (conf->readString(section, "listen_ip", App::LISTEN_IP,
                             Size::IP_V4) ||
            0 == App::LISTEN_IP[0]) {
        ...
        if (conf->readInt(section, "epoll_wait_event_number",
                          &App::EPOLL_WAIT_EVENT_NUM) ||
            0 == App::EPOLL_WAIT_EVENT_NUM) {
            App::EPOLL_WAIT_EVENT_NUM =
                DefaultConfig::App::EPOLL_WAIT_EVENT_NUM;
        }
        ...
        App::TOTAL_SUPPORT_USER_NUM =
            App::NODE_GROUP_NUM * App::NODE_GROUP_SIZE;
    ...
    none_ initializeMsg(CConfReader *conf) {
        ...
        if (conf->readInt(section, "valid_duration", &validDuration) ||
            0 == validDuration) {
            Msg::VALID_DURATION =
                DefaultConfig::Msg::VALID_DURATION * 60 * 60 * 1000000;
        } else {
    ...
    none_ initializeRedis(CConfReader *conf) {
        ...
        if (conf->readShort(section, "port", &Redis::PORT) ||
            0 == Redis::PORT) {
