...
b4_ CConfReader::_isNewline(c1_ c) { return ('\n' == c || '\r' == c) ? 1 : 0; }

b4_ CConfReader::_isEndOfString(c1_ c) { return '\0' == c ? 1 : 0; }

b4_ CConfReader::_isLeftBarce(c1_ c) { return '[' == c ? 1 : 0; }

b4_ CConfReader::_isRightBrace(b1_ c) { return ']' == c ? 1 : 0; }

b4_ CConfReader::_parseFile(const c1_ *section, const c1_ *key, b4_ *secS,
                            b4_ *secE, b4_ *keyS, b4_ *keyE, b4_ *valueS,
                            b4_ *valueE) {
    b4_ i = 0;
    ...
    *secE = *secS = *keyE = *keyS = *valueS = *valueE = -1;
    ...
    while (!_isEndOfString(_buffer[i])) {
        ...
        if ((0 == i || _isNewline(_buffer[i - 1])) &&
            ...
            if (0 ==
                strncmp(_buffer + section_start, section, i - section_start)) {
                i++;
                ...
                *secE = i - 1;
                ...
                while (
                    !(_isNewline(_buffer[i - 1]) && _isLeftBarce(_buffer[i])) &&
