...
CNodeGroup::CNodeGroup()
    : _worker(Config::App::THREAD_STACK_SIZE),
      _queue(Config::App::MESSAGE_MAX_NUM_IN_QUEUE * Message::MSG_MAX_LENGTH,
             &_mutex) {
    _pos = _container.end();
...
bool_ CNodeGroup::putMessage(const Message::TMsg *msg) {
    ...
    if (!_queue.write((const ub1_ *)msg, msg->size)) {
        log_crit(
            "[%p %p]CNodeGroup::putMessage: the size of queue is too "
            "small",
            (obj_)msg->ext, this);
...
bool_ CNodeGroup::_rollingQueue() {
    ...
    log_debug("[%p %p]CNodeGroup::rollingQueue: read a message", (obj_)msg->ext,
              this);
